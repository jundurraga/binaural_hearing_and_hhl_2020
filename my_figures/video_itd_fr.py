...
file_path = '/home/jundurraga/pCloudDrive/Documents/Presentations/international_binaural_workshop_2018/figures/'
...
inch = 2.54
alt_period = 1 / pl['itd_mod_rate']
n_cycles = time[-1] / alt_period
b = alt_period * pl['itd']
ipd_factor_l = np.abs(pl['itd']) / 0.0005
ipd_factor_r = np.abs(pl['alt_itd']) / 0.0005
...
with writer.saving(fig, video_file, 300):
    for _i_frame in range(int(np.floor(pl['duration'] * fps))):
        ...
        ax2.set_xticks([])
        ax2.set_yticks([])
        ax2.set_title('{:.1f} / {:.1f}'.format(pl['itd']*1000, pl['alt_itd']*1000))
...
video_file = file_path + "writer_test.mp4"
output_file = file_path + 'example{:.1f}_{:.1f}'.format(pl['itd']*1000, pl['alt_itd']*1000) + '.mp4'
